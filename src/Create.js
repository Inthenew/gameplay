...
import intruct from './instruct.js';
...
class Create extends Component {
    ...
    async createGame(e) {
        ...
        document.getElementById('getObjects').innerHTML += `<div id="Objects"><input id="osearch" style="width: 150px; display: inline-block;" placeholder="Search" type="text" id="search" class="form-control"><hr/><div id="Objects2"></div><hr/><a id="Play" class="s btn btn-outline-success">Play</a><br/><br/><a id="Save2" class="s btn btn-outline-success">Save</a><br/><br/><a id="Games" class="s btn btn-outline-success">Get Games</a><br/><br/><a id="Settings" class="s btn btn-outline-success">Settings</a></div>`;
        $('#getObjects').css({
            visibility: 'visible'
        })
        ths.cg = true;
    ...
    play() {
        ...
        if (ths.con === 'fp' && !ths.playing) {
            ths.controls.enabled = false;
            let sphereShape = new CANNON.Sphere(1.3);
            let sphereBody = new CANNON.Body({ mass: 5 });
            sphereBody.addShape(sphereShape);
            ths.stp.x = Number(ths.stp.x);
            ths.stp.y = Number(ths.stp.y);
            ths.stp.z = Number(ths.stp.z);
            ths.camera.rotation.set(0, 0, 0);
            ths.camera.position.set(0, 0, 0);
            sphereBody.position.set(ths.stp.x, ths.stp.y, ths.stp.z);
            console.log(ths.stp)
            sphereBody.linearDamping = 0.9;
            ths.world.add(sphereBody);
            ths.pcontrols = new PointerLockControls( ths.camera, sphereBody, ths.stp );
            intruct(document.body, ths.pcontrols);
            ths.pcontrols.enabled = true;
            ths.scene.add( ths.pcontrols.getObject() );
        }
        this.playing = true;
    }
    ...
    setUpScene(width, height, floor) {
        ...
        if (!floor) {
            /** Create Grid **/
            this.scene.add(new THREE.GridHelper(50, 50/** Size, divisions **/));
        }
    ...
    renderScene(floor) {
        if (typeof this.controls !== "undefined") {
            ...
            if (ths.playing && ths.con === 'ob') {
                ths.controls.update();
            } else if (ths.playing && ths.con === 'fp') {
                ths.controls.enabled = false;
                ths.pcontrols.update(Date.now() - ths.time );
            } else if (!ths.playing) {
                ths.controls.update();
            }
